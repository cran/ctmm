...
```
`as.telemetry` also works on `Move` objects, which can be useful as the `move` package interfaces directy with MoveBank through `R`.
...
However, if under any condition the sampling rate changes during data collection, then you will have to account for that in the varioram with the `dt` argument. In the followng example, the collar was programed to cycle between 1, 5, 25 hour sampling intervals
...
If your tracking data is GPS and is annotated with a "DOP" column, then this information is automatically imported into the `telemetry` object and only has to be flagged in the `ctmm` model by setting `error=TRUE`. Because of some convenient mathematical relations, fitting without telemetry errors involves 1-3 fewer parameters and is, therefore, much faster than fitting with telemetry error. So if your data are not swamped by error and do not contain bursts of high-frequency fixes (which would also be relatively swamped by 5 meter GPS error), then you would want to apply a two stage fitting method:
...
where your first `GUESS1` comes from `variogram.fit` and your second `GUESS2` is refined by fitting without errors. This ensures that `optim` gets to the best solution `FIT` within a reasonable amount of steps. On the other hand, if your data is swamped by telemetry errors, then you will need to prepare your best initial guess and fit once with `error=TRUE` flagged. If `isotropic=FALSE`, then you might want to take a few stabs at the orientation and eccentricity of `sigma` to make sure that `optim` has converged on the best solution.
...
If you have a complex model in mind, say `m2`, then you can perform model selection in a more convenient way with the `ctmm.select` function. `ctmm.select` considers the initial guess (hypothesis) and every simpler model to select the best model based upon an information criteria.

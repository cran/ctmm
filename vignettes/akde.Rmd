...
Given that we do not precisely know the autocorrelation parameters of the data, each `akde` object actually contains three kernel density estimates: `ML`, `low`, and `high` corresponding to the maxmimum likelihood optimal smoothing and confidence intervals on the optimal smoothing area. The significance level for these confidence levels can be set with the `alpha` option, which defaults to the conventional `0.05` (see `help(akde)`).
